...
import ClicksHeader from "./ClicksHeader";
...
const ClicksTable = () => {
  ...
  const { isLoading, data } = useQuery({
    queryKey: ["clicks", page, limit, search],
    queryFn: async () => {
      const params = `limit=${limit}&page=${page}&search=${search}&sort=-createdAt`;
      return await getClicks(params);
    },
    placeholderData: keepPreviousData,
  });

  const tableConfig = {
    data: isLoading ? [] : data.clicks,
    columns,
  ...
  return (
    <section className="space-y-6">
      {isLoading ? (
        ...
        <ClicksHeader total={isLoading ? 0 : data?.total} />
      )}
